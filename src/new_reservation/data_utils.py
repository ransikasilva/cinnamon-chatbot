    ...
    for hotel in metadata.get('HotelsAndResorts', []):
        ...
        for room in hotel.get('RoomTypes', []):
            ...
            if room_code:
                ...
                global_room_descriptions[room_code] = room_description  # Last hotel with this code will win
...
def check_room_availability(property_name: str, check_in: str, check_out: str, rooms_needed: int = 1, budget_min: int = 0, budget_max: int = 0) -> List[Dict]:
    """
    ...
    try:
        ...
        date_range = pd.date_range(check_in_date, check_out_date - timedelta(days=1), freq='D')
        ...
        if budget_min > 0 or budget_max > 0:
            ...
            if budget_min > 0 or budget_max > 0:
                budget_str = f" within budget range ${budget_min}-${budget_max}" if budget_min > 0 and budget_max > 0 else \
                           f" above ${budget_min}" if budget_min > 0 else f" under ${budget_max}"
        ...
        print(f"Found {len(available_rooms)} available room types for '{property_name}' from {check_in} to {check_out}")
